...
def train_model(train_df: pd.DataFrame):
    ...
    pipe = Pipeline([("pre", pre), ("clf", RandomForestClassifier(n_estimators=200, max_depth=8, random_state=42))])
...
def render_overview():
    ...
    st.plotly_chart(fig, use_container_width=True, key="chart_risk_count")
    ...
    st.plotly_chart(fig2, use_container_width=True, key="chart_revenue_by_band")
...
def render_segments():
    ...
    if "Contract" in merged_df.columns:
        ...
        st.dataframe(contract_df, use_container_width=True, hide_index=True, key="table_by_contract")
    ...
    if "InternetService" in merged_df.columns:
        ...
        st.dataframe(internet_df, use_container_width=True, hide_index=True, key="table_by_internet")
...
def render_model_performance():
    ...
    with col1:
        ...
        st.plotly_chart(fig, use_container_width=True, key="chart_confusion_matrix")
    ...
    with col2:
        if metrics["roc_curve"] is not None:
            ...
            st.plotly_chart(fig2, use_container_width=True, key="chart_roc_curve")
    ...
    st.plotly_chart(fig3, use_container_width=True, key="chart_feature_importance")
...
def render_sql_explorer():
    ...
    if st.button("Run Query", key="sql_run_button"):
        try:
            ...
            st.dataframe(result, use_container_width=True, key="table_sql_result")
